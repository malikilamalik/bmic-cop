...
/// Init `MySqlJobDetailRepository` from `MysqlConfig` (connects to `datamart`).
/// Uses `pkg::mysql::init` lazy pool — does not require live DB at init time.
pub async fn init(config: &MysqlConfig) -> Result<MySqlJobDetailRepository, sqlx::Error> {
    let pool = pkg::mysql::init(config).database("datamart").await?;
    Ok(MySqlJobDetailRepository::new(pool))
}

/// Init from an existing pool (useful for tests / DI).
pub fn init_with_pool(pool: DbPool) -> MySqlJobDetailRepository {
    MySqlJobDetailRepository::new(pool)
}
