...
/// MySQL `NOW()`.
const LIST_ALL_SQL: &str = "SELECT id, name, start_valid_date, end_valid_date, is_active, running_frequency, created_at, created_by, updated_at, updated_by, deleted_at, deleted_by FROM evaluator WHERE deleted_at IS NULL AND (start_valid_date IS NULL OR start_valid_date <= ?) AND (end_valid_date IS NULL OR end_valid_date >= ?)";

const LIST_BY_ID_SQL: &str = "SELECT id, name, start_valid_date, end_valid_date, is_active, running_frequency, created_at, created_by, updated_at, updated_by, deleted_at, deleted_by FROM evaluator WHERE deleted_at IS NULL AND id = ? AND (start_valid_date IS NULL OR start_valid_date <= ?) AND (end_valid_date IS NULL OR end_valid_date >= ?)";
