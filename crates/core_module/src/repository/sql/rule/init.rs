...
/// Init `MySqlRuleRepository` from `MysqlConfig` (connects to `business_rule`).
/// Uses `pkg::mysql::init` lazy pool — does not require live DB at init time.
pub async fn init(config: &MysqlConfig) -> Result<MySqlRuleRepository, sqlx::Error> {
    let pool = pkg::mysql::init(config).database("business_rule").await?;
    Ok(MySqlRuleRepository::new(pool))
}

/// Init from an existing pool (useful for tests / DI).
pub fn init_with_pool(pool: DbPool) -> MySqlRuleRepository {
    MySqlRuleRepository::new(pool)
}
